...
func TestSetupTestCatalog(t *testing.T) {
	_ = SetupTest(t)
	...
	req = `
		{
			...
			"spec": {
				"rules": [{
					"intent": "Allow",
					"actions": ["kubernetes.pods.list", "kubernetes.deployments.restart", "kubernetes.troubleshoot"],
	...
	req = `
		{
			...
			"spec": {
				"rules": [{
					"intent": "Allow",
					"actions": ["kubernetes.pods.list", "kubernetes.troubleshoot"],
					"targets": ["res://skillsets/skillsets/kubernetes-demo"]
				}]
			}
		}`
