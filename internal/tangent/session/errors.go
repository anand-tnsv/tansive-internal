...
var (
	...
	ErrTokenRequired                apperrors.Error = apperrors.New("token is required").SetStatusCode(http.StatusBadRequest)
	ErrTokenExpired                 apperrors.Error = apperrors.New("token has expired").SetStatusCode(http.StatusBadRequest)
	ErrFailedRequestToTansiveServer apperrors.Error = apperrors.New("failed to make request to Tansive server").SetStatusCode(http.StatusInternalServerError)
)
