...
// 	entries := make(map[string]*HashedLogEntry)
// 	children := make(map[string][]*HashedLogEntry)
// 	var roots []*HashedLogEntry
...
// 			continue
// 		}
// 		id := str(entry.Payload["invocation_id"])
// 		parent := str(entry.Payload["invoker_id"])
// 		entries[id] = &entry
// 		if parent != "" {
...
// 	writeHeader(out, sessionID)
// 	for _, root := range roots {
// 		renderEntryRecursive(out, root, children, 0)
// 	}
// 	fmt.Fprint(out, "</body></html>")
// 	return nil
// }

// func renderEntryRecursive(out io.Writer, entry *HashedLogEntry, children map[string][]*HashedLogEntry, depth int) {
// 	p := entry.Payload
// 	skill := str(p["skill"])
// 	event := strings.ToUpper(str(p["event"]))
// 	timeStr := ""
...
// 			timeStr = parsed.Local().Format("2006-01-02 15:04:05 MST")
// 		} else {
// 			timeStr = t
// 		}
// 	}

// 	fmt.Fprintf(out, `<details open style="margin-left:%dpx"><summary>🧠 %s (%s)</summary>`, depth*20, html.EscapeString(skill), html.EscapeString(event))
// 	fmt.Fprint(out, `<div class="entry">`)
// 	if timeStr != "" {
// 		fmt.Fprintf(out, `<div class="time">%s</div>`, html.EscapeString(timeStr))
// 	}

// 	writeField(out, "Event", event)
// 	writeField(out, "Skill Name", str(p["skill_name"]))
// 	writeField(out, "Actor", str(p["actor"]))
// 	writeField(out, "Runner", str(p["runner"]))
// 	writeField(out, "Message", str(p["message"]))
// 	writeField(out, "View", str(p["view"]))
// 	writeField(out, "Decision", strings.ToUpper(str(p["decision"])))
// 	if status := str(p["status"]); status != "" {
// 		writeField(out, "Status", status)
// 	}
// 	if errVal, ok := p["error"]; ok {
// 		fmt.Fprintf(out, `<div class="error"><strong>Error:</strong> %s</div>`, html.EscapeString(fmt.Sprintf("%v", errVal)))
// 	}
// 	if args, ok := p["input_args"]; ok {
// 		if b, err := json.MarshalIndent(args, "", "  "); err == nil {
// 			fmt.Fprintf(out, `<div class="input"><strong>Input Args:</strong><br><pre>%s</pre></div>`, html.EscapeString(string(b)))
// 		}
// 	}
// 	if basis, ok := p["basis"]; ok {
// 		if b, err := json.MarshalIndent(basis, "", "  "); err == nil {
...
// 	for _, child := range children[str(p["invocation_id"])] {
// 		renderEntryRecursive(out, child, children, depth+1)
// 	}
...
// 	if value != "" {
// 		fmt.Fprintf(out, `<div class="field"><span class="label">%s:</span><span class="value">%s</span></div>`, html.EscapeString(label), html.EscapeString(value))
// 	}
// }

// func writeHeader(out io.Writer, sessionID string) {
// 	fmt.Fprint(out, `<html><head><meta charset="UTF-8"><title>Tansive™ Session Log</title>
// <style>
// body { font-family: sans-serif; margin: 2em; background: #fff; color: #000; }
// h1 { font-size: 1.6em; margin-bottom: 0.3em; }
// h2 { font-weight: normal; color: #888; font-size: 1em; margin-bottom: 1.5em; }
// .entry { border-left: 4px solid #ccc; margin: 1em 0; padding: 1em; background: #fefefe; }
// @media (prefers-color-scheme: dark) {
//  body { background: #111; color: #ccc; }
//  .entry { background: #1b1b1b; border-left-color: #444; }
// }
// .time { font-size: 0.9em; color: #888; margin-bottom: 0.5em; }
// .label { font-weight: 600; }
// .value { margin-left: 0.5em; color: #bbb; font-weight: normal; }
// .input, .error, .basis { margin-top: 0.75em; font-family: monospace; white-space: pre-wrap; }
// pre { background: #222; color: #ddd; padding: 0.5em; border-radius: 4px; overflow-x: auto; }
// @media (prefers-color-scheme: light) {
//  pre { background: #f6f8fa; color: #333; }
// }
// details summary { font-size: 1em; font-weight: bold; cursor: pointer; margin-bottom: 0.5em; }
// </style>
// </head><body>
// <h1>Tansive™ Session Log</h1>
