...
func shellCmd(ctx context.Context, channel *channel, script string) apperrors.Error {
	cmd, err := createCommand(ctx, script, &cmdIOStreams{
		out: os.Stdout,
		err: os.Stderr,
	}, channel)
	if err != nil {
...
func createCommand(ctx context.Context, script string, s *cmdIOStreams, c *channel) (*exec.Cmd, apperrors.Error) {
	if c.shellContext == nil {
		return nil, ErrSessionError.Msg("shell context is not set")
	}

	if err := os.MkdirAll(c.shellContext.dir, 0755); err != nil {
		return nil, ErrExecutionFailed.Msg("failed to create working directory: " + err.Error())
	}

	cmd := exec.CommandContext(ctx, "/bin/bash", "-c", script)
	cmd.Dir = c.shellContext.dir

	baseEnv := os.Environ()
	env := appendOrReplaceEnv(baseEnv, "HOME", c.shellContext.dir)
	cmd.Env = env
